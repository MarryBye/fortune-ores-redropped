...
public class OreGenSuppressor {
    ...
    private static synchronized void buildBlockTable() {
        ...
        for (Ore ore : FortuneOres.oreStorage) {
            ...
            for (String oreName : ore.oreNames) {
                // "oreNether*" means "this ore, but double" - a different ore type, generated by the mod that owns it.
                // Denying it here would silently delete Nether Ores' generation, which no EnableOreGen switch asked
                // for; the event path above skips the same names.
                if (oreName.contains("Nether")) continue;
